...
#CASO CON AVO --> Único que nos interesa para escoger zonas
def weightfun_AVO(n1, n2, arco):
	arco = arco[0]
	if "name" in arco:
...
for key in Matriz_OD:
	...
	for i, node in enumerate(G.nodes):
		...
		if dist_nodo < distancia_minima:
			...
			nodo_destino = node
	try:
		take_paths = list(nx.all_shortest_paths(G, nodo_origen, nodo_destino, weight=weightfun_AVO))
		#print(take_paths)
